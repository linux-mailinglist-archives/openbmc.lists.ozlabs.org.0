Return-Path: <openbmc-bounces+lists+openbmc=lfdr.de@lists.ozlabs.org>
X-Original-To: lists+openbmc@lfdr.de
Delivered-To: lists+openbmc@lfdr.de
Received: from lists.ozlabs.org (lists.ozlabs.org [112.213.38.117])
	by mail.lfdr.de (Postfix) with ESMTPS id 54E3F3AA8DB
	for <lists+openbmc@lfdr.de>; Thu, 17 Jun 2021 04:02:55 +0200 (CEST)
Received: from boromir.ozlabs.org (localhost [IPv6:::1])
	by lists.ozlabs.org (Postfix) with ESMTP id 4G551B0F0Rz3brt
	for <lists+openbmc@lfdr.de>; Thu, 17 Jun 2021 12:02:54 +1000 (AEST)
Authentication-Results: lists.ozlabs.org;
	dkim=pass (1024-bit key; unprotected) header.d=nuvoton.onmicrosoft.com header.i=@nuvoton.onmicrosoft.com header.a=rsa-sha256 header.s=selector2-nuvoton-onmicrosoft-com header.b=uOusxUpn;
	dkim-atps=neutral
X-Original-To: openbmc@lists.ozlabs.org
Delivered-To: openbmc@lists.ozlabs.org
Authentication-Results: lists.ozlabs.org; spf=pass (sender SPF authorized)
 smtp.helo=apc01-sg2-obe.outbound.protection.outlook.com
 (client-ip=2a01:111:f400:febd::61d;
 helo=apc01-sg2-obe.outbound.protection.outlook.com;
 envelope-from=chli30@nuvoton.com; receiver=<UNKNOWN>)
Authentication-Results: lists.ozlabs.org; dkim=pass (1024-bit key;
 unprotected) header.d=nuvoton.onmicrosoft.com
 header.i=@nuvoton.onmicrosoft.com header.a=rsa-sha256
 header.s=selector2-nuvoton-onmicrosoft-com header.b=uOusxUpn; 
 dkim-atps=neutral
Received: from APC01-SG2-obe.outbound.protection.outlook.com
 (mail-sg2apc01on061d.outbound.protection.outlook.com
 [IPv6:2a01:111:f400:febd::61d])
 (using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
 (No client certificate requested)
 by lists.ozlabs.org (Postfix) with ESMTPS id 4G550q5jclz2xZp
 for <openbmc@lists.ozlabs.org>; Thu, 17 Jun 2021 12:02:34 +1000 (AEST)
ARC-Seal: i=1; a=rsa-sha256; s=arcselector9901; d=microsoft.com; cv=none;
 b=
ARC-Message-Signature: i=1; a=rsa-sha256; c=relaxed/relaxed; d=microsoft.com; 
 s=arcselector9901;
 h=From:Date:Subject:Message-ID:Content-Type:MIME-Version:X-MS-Exchange-SenderADCheck;
 bh=9WUHk8KygdG9sxXGPhLkUlDrntDkWKPFLK5/yHzREh0=;
 b=
ARC-Authentication-Results: i=1; mx.microsoft.com 1; spf=pass
 smtp.mailfrom=nuvoton.com; dmarc=pass action=none header.from=nuvoton.com;
 dkim=pass header.d=nuvoton.com; arc=none
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
 d=nuvoton.onmicrosoft.com; s=selector2-nuvoton-onmicrosoft-com;
 h=From:Date:Subject:Message-ID:Content-Type:MIME-Version:X-MS-Exchange-SenderADCheck;
 bh=9WUHk8KygdG9sxXGPhLkUlDrntDkWKPFLK5/yHzREh0=;
 b=uOusxUpnf4IHv0x+OuBjJqyAn3Qfb7xoidVhSC6dfSkoB3DuuahOVkNTbyux4PM0AFzO31hWGVYDGvbrjDrI9DkPcBCIE85H+dFOAo16J2zjRilt8bvWhxhUec0rSWPBju9Io6mWO0e272GBgvNmZxUa0CTtYPzEDNu+UVZmKeA=
Received: from HK0PR03MB5090.apcprd03.prod.outlook.com (2603:1096:203:bf::11)
 by HK0PR03MB3233.apcprd03.prod.outlook.com (2603:1096:203:4c::11)
 with Microsoft SMTP Server (version=TLS1_2,
 cipher=TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384) id 15.20.4264.7; Thu, 17 Jun
 2021 02:02:27 +0000
Received: from HK0PR03MB5090.apcprd03.prod.outlook.com
 ([fe80::3571:ddd6:60f9:d0db]) by HK0PR03MB5090.apcprd03.prod.outlook.com
 ([fe80::3571:ddd6:60f9:d0db%7]) with mapi id 15.20.4242.018; Thu, 17 Jun 2021
 02:02:27 +0000
From: CS20 CHLi30 <CHLI30@nuvoton.com>
To: Rahul Maheshwari <rahulmaheshwari01@gmail.com>
Subject: RE: [OpenBMC][Test Automation] Test Bmc Dump test failed
Thread-Topic: [OpenBMC][Test Automation] Test Bmc Dump test failed
Thread-Index: AddNLUo0hqadIm8eSdqVqlNp76OXdwECbkGABHizZzA=
Date: Thu, 17 Jun 2021 02:02:27 +0000
Message-ID: <HK0PR03MB5090745CEB1A7183545A43E4CF0E9@HK0PR03MB5090.apcprd03.prod.outlook.com>
References: <HK0PR03MB5090567C78942F49806377D4CF2A9@HK0PR03MB5090.apcprd03.prod.outlook.com>
 <CAAMkS11HNMhL6HmNq48jOSiV7pPTfFW-XmKH9fepSPScNO8d4g@mail.gmail.com>
In-Reply-To: <CAAMkS11HNMhL6HmNq48jOSiV7pPTfFW-XmKH9fepSPScNO8d4g@mail.gmail.com>
Accept-Language: zh-TW, en-US
Content-Language: en-US
X-MS-Has-Attach: yes
X-MS-TNEF-Correlator: 
authentication-results: gmail.com; dkim=none (message not signed)
 header.d=none;gmail.com; dmarc=none action=none header.from=nuvoton.com;
x-originating-ip: [60.250.207.217]
x-ms-publictraffictype: Email
x-ms-office365-filtering-correlation-id: e6753f4b-39db-41f2-d492-08d93133f522
x-ms-traffictypediagnostic: HK0PR03MB3233:
x-ms-exchange-transport-forked: True
x-microsoft-antispam-prvs: <HK0PR03MB3233D67ABDB7038BDB8BFA18CF0E9@HK0PR03MB3233.apcprd03.prod.outlook.com>
x-ms-oob-tlc-oobclassifiers: OLM:8273;
x-ms-exchange-senderadcheck: 1
x-microsoft-antispam: BCL:0;
x-microsoft-antispam-message-info: 
x-forefront-antispam-report: CIP:255.255.255.255; CTRY:; LANG:en; SCL:1; SRV:;
 IPV:NLI; SFV:NSPM; H:HK0PR03MB5090.apcprd03.prod.outlook.com; PTR:; CAT:NONE;
 SFS:(4636009)(346002)(39850400004)(396003)(376002)(136003)(366004)(83380400001)(38100700002)(166002)(966005)(478600001)(186003)(66574015)(26005)(99936003)(6506007)(53546011)(122000001)(4326008)(54906003)(86362001)(316002)(33656002)(5660300002)(76116006)(7696005)(55016002)(2906002)(6916009)(52536014)(8676002)(66556008)(8936002)(71200400001)(66616009)(64756008)(66946007)(66476007)(9686003)(66446008);
 DIR:OUT; SFP:1101; 
x-ms-exchange-antispam-messagedata-chunkcount: 1
x-ms-exchange-antispam-messagedata-0: =?us-ascii?Q?9Q5MyE79tahRDX59uBDVJ2ts+upH9OX1gVwQss5OyvaBdR35DLDFrgLeV+z8?=
 =?us-ascii?Q?h1fkd0V/GCgWMF/59SrGOynU8Ew0bJarNs4nwfLxum/vB7yL71hwOt54pXR/?=
 =?us-ascii?Q?KmVocTmWME7wcy2Wc1xOCqdPXtcFnR5e7NyAF09LTkPvO12gC8qEVb2PmCKq?=
 =?us-ascii?Q?e0N5Z1VxWFDaHlRXiaX21WxcABknuFkq1T90TKm40IGMN0ptpZJAABp57Unz?=
 =?us-ascii?Q?pxckU3NzULiEH5xlmt3RoOYSOqISXYDUQ2Tz5tFXaR6Wu1dxhubuZi5Qnsbm?=
 =?us-ascii?Q?QIUwdRiMMqdATzcOH/Lz9iS+nZHQ1r7cKpJevN+74sLx4PXwDg8d28ZuBkGE?=
 =?us-ascii?Q?MmOa5Ao1fkMxH3lKe+yEoKz/vpoN6z9hNnUrRlaLvkde4X6DpMZTVN4fO9Ft?=
 =?us-ascii?Q?J9A+/2ruB8BFjecDyLEukvgrDdfVcNz7V5nIHa5Imykk/OTXb/mb2HZ9BrT/?=
 =?us-ascii?Q?4IZjz3ZoujoyoCt6iX5gOUAKhou7oWSPwB1zDktzXwt8QuDGKINuBgf0v/Uo?=
 =?us-ascii?Q?HKoHcFFAnvgU13CYQz/DBuVeQNGaJwPam6cuz5LaAWlMvVO7YRjbSQpC0nTS?=
 =?us-ascii?Q?8NAJHr5HNGSJl+waTdA4tMVfDul/AKCHhD4adaNGrjmlwXQZN2VvCwJN9DEh?=
 =?us-ascii?Q?szVq6K8nJuMNcP/E5nb5A1xoUDbgYn0J8YhjOtviU/Vbff1M7QTGWs8heQX6?=
 =?us-ascii?Q?0S7V0QOK2wBqx72nDTQNVcdRBxh9JFrMWAmywLbY7/tHCHZBpY/LmiqNg2pP?=
 =?us-ascii?Q?IeRhSaMuYk5J4b7zEsiLHEW6ED3i9ogv24dWezJ6vqxm3aA460BXQxJMDNBj?=
 =?us-ascii?Q?yzeHPfp9epFdM+MptfVY/l+t6mVdcGMwP/tL4JtlRA5747NFPJ+EK3ZhPeRp?=
 =?us-ascii?Q?O6cgUwG1wbkLbbJH83fg4YIff/M7/IavJQG37Zwq+wcYEhSbt8cT2Dlxps5y?=
 =?us-ascii?Q?Xp355h+tSAm4VJe6vTBqq8X1wQ1j1vjGceCqElgXiMmLv+MtN84c17s/ldVf?=
 =?us-ascii?Q?E4om3GCdnfPCojHC4d5eBYvqVkBXFHv6N5K4HlIZfIjkZoc8MAec9DcJ/m9n?=
 =?us-ascii?Q?Un7Sve9VFF/OxxIwdeJyS7A0Qh2W/VYIW2rOVfOsNHztJu/2bk5P1+IRZwki?=
 =?us-ascii?Q?reolij2MsMQst+yyFOS0n39vYybQM1E5i53+HnDpO4Dz3xJ1W91NeS8UF9qY?=
 =?us-ascii?Q?ICQAZlVBRMV6KI43okPmqCzeA4gRvQFMFmUAFm6mL8QHMu4dEsGZogJ5luMh?=
 =?us-ascii?Q?KCbJozgXZ0isStzvKVPGtMmnoDzgRlOnqWaBbm4LtF/DdlcX+JDUxY+Yk96q?=
 =?us-ascii?Q?D/s7/ncC62xEjJfxBIi1WRFY?=
Content-Type: multipart/related;
 boundary="_004_HK0PR03MB5090745CEB1A7183545A43E4CF0E9HK0PR03MB5090apcp_";
 type="multipart/alternative"
MIME-Version: 1.0
X-OriginatorOrg: nuvoton.com
X-MS-Exchange-CrossTenant-AuthAs: Internal
X-MS-Exchange-CrossTenant-AuthSource: HK0PR03MB5090.apcprd03.prod.outlook.com
X-MS-Exchange-CrossTenant-Network-Message-Id: e6753f4b-39db-41f2-d492-08d93133f522
X-MS-Exchange-CrossTenant-originalarrivaltime: 17 Jun 2021 02:02:27.3910 (UTC)
X-MS-Exchange-CrossTenant-fromentityheader: Hosted
X-MS-Exchange-CrossTenant-id: a3f24931-d403-4b4a-94f1-7d83ac638e07
X-MS-Exchange-CrossTenant-mailboxtype: HOSTED
X-MS-Exchange-CrossTenant-userprincipalname: WztKLqg+yLHZNjndMjhm6I70CHgiG3i7Eh5OkD0tziwud9W57UppX9gNJA/HO0qxnrqMSl1eniN8tqurAXJhNw==
X-MS-Exchange-Transport-CrossTenantHeadersStamped: HK0PR03MB3233
X-BeenThere: openbmc@lists.ozlabs.org
X-Mailman-Version: 2.1.29
Precedence: list
List-Id: Development list for OpenBMC <openbmc.lists.ozlabs.org>
List-Unsubscribe: <https://lists.ozlabs.org/options/openbmc>,
 <mailto:openbmc-request@lists.ozlabs.org?subject=unsubscribe>
List-Archive: <http://lists.ozlabs.org/pipermail/openbmc/>
List-Post: <mailto:openbmc@lists.ozlabs.org>
List-Help: <mailto:openbmc-request@lists.ozlabs.org?subject=help>
List-Subscribe: <https://lists.ozlabs.org/listinfo/openbmc>,
 <mailto:openbmc-request@lists.ozlabs.org?subject=subscribe>
Cc: CS20 KWLiu <KWLIU@nuvoton.com>,
 "openbmc@lists.ozlabs.org" <openbmc@lists.ozlabs.org>
Errors-To: openbmc-bounces+lists+openbmc=lfdr.de@lists.ozlabs.org
Sender: "openbmc" <openbmc-bounces+lists+openbmc=lfdr.de@lists.ozlabs.org>

--_004_HK0PR03MB5090745CEB1A7183545A43E4CF0E9HK0PR03MB5090apcp_
Content-Type: multipart/alternative;
	boundary="_000_HK0PR03MB5090745CEB1A7183545A43E4CF0E9HK0PR03MB5090apcp_"

--_000_HK0PR03MB5090745CEB1A7183545A43E4CF0E9HK0PR03MB5090apcp_
Content-Type: text/plain; charset="us-ascii"
Content-Transfer-Encoding: quoted-printable

Hi Rahul,
After using redfish create dump, we got all test items failed from redfish/=
managers/test_bmc_dumps.robot.

And we found that problem in bmcweb and test_bmc_dumps.robot. Thus, we have=
 provided patches to fix this problem then test is PASS.
I also add you as reviewer, could you please help to check this issue and o=
ur patches? Thanks you.

Patch for test_bmc_dumps.robot:
https://gerrit.openbmc-project.xyz/c/openbmc/openbmc-test-automation/+/4416=
1
redfish: managers: Fix extract dump id from correct dump location URI

Symptom:
Create User Initiated BMC Dump keyword return incorrect field

Root cause:
"Create User Initiated BMC Dump" HttpHeaders of Payload didn't return corre=
ct location
then cause dump id cannot get properly

Solution:
Extract dump id from correct dump location URI

Tested:
robot test PASS for "Test Bmc Dumps" from redfish/managers/test_bmc_dumps.r=
obot

Patch for bmcweb:
https://gerrit.openbmc-project.xyz/c/openbmc/bmcweb/+/44006
redfish: log_services: fix createDump functionality of redfish

Symptom:
CreateDump resp_handler got error in bmcweb then report internal error.

Root cause:
1. createDump() function in bmcweb didn't support parameters as CreateDump(=
) function
   in phosphor-debug-collector then cause create dump through redfish all t=
est items are failed.
2. createTask() function called by createDumpTaskCallback() in bmcweb didn'=
t handle well with
   member InterfacesAdded then cause match rule didn't match and return dbu=
s error.

Solution:
1. Modify sync_method_call with object_path return type for CreateDump meth=
od call.
2. Modify match rule with member PropertiesChanged for bmc dump entry path.

Tested:
curl -k -H "X-Auth-Token: $token" -X POST https://${bmc}/redfish/v1/Manager=
s/bmc/LogServices/Dump/Actions/LogService.CollectDiagnosticData
-d '{"DiagnosticDataType": "Manager"}'

Sincerely,
Tim
From: openbmc [mailto:openbmc-bounces+chli30=3Dnuvoton.com@lists.ozlabs.org=
] On Behalf Of Rahul Maheshwari
Sent: Tuesday, May 25, 2021 3:25 PM
To: CS20 CHLi30 <CHLI30@nuvoton.com>
Cc: CS20 KWLiu <KWLIU@nuvoton.com>; openbmc@lists.ozlabs.org
Subject: Re: [OpenBMC][Test Automation] Test Bmc Dump test failed

Hi Tim
`/redfish/extended/test_bmc_dump.robot` suite has usage of older xyz dump p=
ath. Can you make use of `redfish/managers/test_bmc_dumps.robot` suite to r=
un the dump automation test? This suite has usage of recent Redfish dump ur=
i implementation i.e. /redfish/v1/Managers/bmc/LogServices/Dump.

Thanks
Rahul

On Thu, May 20, 2021 at 10:00 AM CS20 CHLi30 <CHLI30@nuvoton.com<mailto:CHL=
I30@nuvoton.com>> wrote:
Hi All,

We found that some of "Test Bmc Dump" items are failed when we run test aut=
omation.
After checking log and source code that seems relate to return type of Crea=
teDump() in phosphor-debug-collector
is not consistent with OpenBMC-Test-Automation/redfish/extended/test_bmc_du=
mp.robot.

Seems we need to modify test_bmc_dump.robot about return type from "Create =
User Initiated Dump" keyword
from ${dump_id} to object_path according current CreateDump() implementatio=
n in phosphor-debug-collector, am I right?

Summary:
CreateDump() change return type from uint32_t to object_path from this comm=
it:
https://github.com/openbmc/phosphor-debug-collector/commit/6ccb50e133342d20=
d2758c3efc3f8b429abcbf90#diff-6053a5177ee8dcedec88c55df4de554b5a54497f92b04=
1e1585c6770236773a1<https://apc01.safelinks.protection.outlook.com/?url=3Dh=
ttps%3A%2F%2Fgithub.com%2Fopenbmc%2Fphosphor-debug-collector%2Fcommit%2F6cc=
b50e133342d20d2758c3efc3f8b429abcbf90%23diff-6053a5177ee8dcedec88c55df4de55=
4b5a54497f92b041e1585c6770236773a1&data=3D04%7C01%7Cchli30%40nuvoton.com%7C=
00069a126c024c78854208d91f4e4631%7Ca3f24931d4034b4a94f17d83ac638e07%7C0%7C0=
%7C637575943652844341%7CUnknown%7CTWFpbGZsb3d8eyJWIjoiMC4wLjAwMDAiLCJQIjoiV=
2luMzIiLCJBTiI6Ik1haWwiLCJXVCI6Mn0%3D%7C1000&sdata=3DToC4ABFr7NkmzgFIJWC0HX=
oO6FGKe15gqZW345jvIec%3D&reserved=3D0>

However, in OpenBMC-Test-Automation/redfish/extended/test_bmc_dump.robot
Verify User Initiated BMC Dump When Powered Off
    [Documentation]  Create user initiated BMC dump at host off state and
    ...  verify dump entry for it.
    [Tags]  Verify_User_Initiated_BMC_Dump_When_Powered_Off

    Redfish Power Off  stack_mode=3Dskip
    ${dump_id}=3D  Create User Initiated Dump
    Check Existence Of BMC Dump File  ${dump_id}

Run keyword "Create User Initiated Dump" still using dump id (uint32_t) to =
check create bmc dump functionality.
Thus, we got error message from "Check Existence Of BMC Dump File  ${dump_i=
d}" as below:
Evaluating expression '/xyz/openbmc_project/dump/bmc/entry/1 =3D=3D None' f=
ailed: SyntaxError: invalid syntax.

Repo steps:
Run test automation: robot -t Verify_User_Initiated_BMC_Dump_When_Powered_O=
ff redfish/extended/test_bmc_dump.robot

Symptom:
screenshot from test automation report for you refer it.
[cid:image001.jpg@01D7635D.7AA549C0]

Sincerely,
Tim
________________________________
The privileged confidential information contained in this email is intended=
 for use only by the addressees as indicated by the original sender of this=
 email. If you are not the addressee indicated in this email or are not res=
ponsible for delivery of the email to such a person, please kindly reply to=
 the sender indicating this fact and delete all copies of it from your comp=
uter and network server immediately. Your cooperation is highly appreciated=
. It is advised that any unauthorized use of confidential information of Nu=
voton is strictly prohibited; and any information in this email irrelevant =
to the official business of Nuvoton shall be deemed as neither given nor en=
dorsed by Nuvoton.
________________________________
The privileged confidential information contained in this email is intended=
 for use only by the addressees as indicated by the original sender of this=
 email. If you are not the addressee indicated in this email or are not res=
ponsible for delivery of the email to such a person, please kindly reply to=
 the sender indicating this fact and delete all copies of it from your comp=
uter and network server immediately. Your cooperation is highly appreciated=
. It is advised that any unauthorized use of confidential information of Nu=
voton is strictly prohibited; and any information in this email irrelevant =
to the official business of Nuvoton shall be deemed as neither given nor en=
dorsed by Nuvoton.

--_000_HK0PR03MB5090745CEB1A7183545A43E4CF0E9HK0PR03MB5090apcp_
Content-Type: text/html; charset="us-ascii"
Content-Transfer-Encoding: quoted-printable

<html xmlns:v=3D"urn:schemas-microsoft-com:vml" xmlns:o=3D"urn:schemas-micr=
osoft-com:office:office" xmlns:w=3D"urn:schemas-microsoft-com:office:word" =
xmlns:m=3D"http://schemas.microsoft.com/office/2004/12/omml" xmlns=3D"http:=
//www.w3.org/TR/REC-html40">
<head>
<meta http-equiv=3D"Content-Type" content=3D"text/html; charset=3Dus-ascii"=
>
<meta name=3D"Generator" content=3D"Microsoft Word 15 (filtered medium)">
<!--[if !mso]><style>v\:* {behavior:url(#default#VML);}
o\:* {behavior:url(#default#VML);}
w\:* {behavior:url(#default#VML);}
.shape {behavior:url(#default#VML);}
</style><![endif]--><style><!--
/* Font Definitions */
@font-face
	{font-family:PMingLiU;
	panose-1:2 2 5 0 0 0 0 0 0 0;}
@font-face
	{font-family:"Cambria Math";
	panose-1:2 4 5 3 5 4 6 3 2 4;}
@font-face
	{font-family:Calibri;
	panose-1:2 15 5 2 2 2 4 3 2 4;}
@font-face
	{font-family:PMingLiU;
	panose-1:2 1 6 1 0 1 1 1 1 1;}
/* Style Definitions */
p.MsoNormal, li.MsoNormal, div.MsoNormal
	{margin:0cm;
	margin-bottom:.0001pt;
	font-size:12.0pt;
	font-family:"Times New Roman",serif;}
a:link, span.MsoHyperlink
	{mso-style-priority:99;
	color:blue;
	text-decoration:underline;}
a:visited, span.MsoHyperlinkFollowed
	{mso-style-priority:99;
	color:purple;
	text-decoration:underline;}
p.msonormal0, li.msonormal0, div.msonormal0
	{mso-style-name:msonormal;
	mso-margin-top-alt:auto;
	margin-right:0cm;
	mso-margin-bottom-alt:auto;
	margin-left:0cm;
	font-size:12.0pt;
	font-family:"Times New Roman",serif;}
span.gmail-il
	{mso-style-name:gmail-il;}
span.EmailStyle19
	{mso-style-type:personal-reply;
	font-family:"Calibri",sans-serif;
	color:#1F497D;}
.MsoChpDefault
	{mso-style-type:export-only;
	font-family:"Calibri",sans-serif;}
@page WordSection1
	{size:612.0pt 792.0pt;
	margin:72.0pt 90.0pt 72.0pt 90.0pt;}
div.WordSection1
	{page:WordSection1;}
--></style><!--[if gte mso 9]><xml>
<o:shapedefaults v:ext=3D"edit" spidmax=3D"1026" />
</xml><![endif]--><!--[if gte mso 9]><xml>
<o:shapelayout v:ext=3D"edit">
<o:idmap v:ext=3D"edit" data=3D"1" />
</o:shapelayout></xml><![endif]-->
</head>
<body lang=3D"ZH-TW" link=3D"blue" vlink=3D"purple">
<div class=3D"WordSection1">
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">Hi Rahul,<o:p></o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">After using redfish create dump, we got=
 all test items failed from redfish/managers/test_bmc_dumps.robot.<o:p></o:=
p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D"><o:p>&nbsp;</o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">And we found that problem in bmcweb and=
 test_bmc_dumps.robot. Thus, we have provided patches to fix this problem t=
hen test is PASS.<o:p></o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">I also add you as reviewer, could you p=
lease help to check this issue and our patches? Thanks you.<o:p></o:p></spa=
n></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D"><o:p>&nbsp;</o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">Patch for test_bmc_dumps.robot:<o:p></o=
:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D"><a href=3D"https://gerrit.openbmc-proje=
ct.xyz/c/openbmc/openbmc-test-automation/+/44161">https://gerrit.openbmc-pr=
oject.xyz/c/openbmc/openbmc-test-automation/+/44161</a><o:p></o:p></span></=
p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">redfish: managers: Fix extract dump id =
from correct dump location URI<o:p></o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D"><o:p>&nbsp;</o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">Symptom:<o:p></o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">Create User Initiated BMC Dump keyword =
return incorrect field<o:p></o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D"><o:p>&nbsp;</o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">Root cause:<o:p></o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">&quot;Create User Initiated BMC Dump&qu=
ot; HttpHeaders of Payload didn't return correct location<o:p></o:p></span>=
</p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">then cause dump id cannot get properly<=
o:p></o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D"><o:p>&nbsp;</o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">Solution:<o:p></o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">Extract dump id from correct dump locat=
ion URI<o:p></o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D"><o:p>&nbsp;</o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">Tested:<o:p></o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">robot test PASS for &quot;Test Bmc Dump=
s&quot; from redfish/managers/test_bmc_dumps.robot<o:p></o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D"><o:p>&nbsp;</o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">Patch for bmcweb:<o:p></o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D"><a href=3D"https://gerrit.openbmc-proje=
ct.xyz/c/openbmc/bmcweb/+/44006">https://gerrit.openbmc-project.xyz/c/openb=
mc/bmcweb/+/44006</a><o:p></o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">redfish: log_services: fix createDump f=
unctionality of redfish<o:p></o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D"><o:p>&nbsp;</o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">Symptom:<o:p></o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">CreateDump resp_handler got error in bm=
cweb then report internal error.<o:p></o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D"><o:p>&nbsp;</o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">Root cause:<o:p></o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">1. createDump() function in bmcweb didn=
't support parameters as CreateDump() function<o:p></o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">&nbsp;&nbsp; in phosphor-debug-collecto=
r then cause create dump through redfish all test items are failed.<o:p></o=
:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">2. createTask() function called by crea=
teDumpTaskCallback() in bmcweb didn't handle well with<o:p></o:p></span></p=
>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">&nbsp;&nbsp; member InterfacesAdded the=
n cause match rule didn't match and return dbus error.<o:p></o:p></span></p=
>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D"><o:p>&nbsp;</o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">Solution:<o:p></o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">1. Modify sync_method_call with object_=
path return type for CreateDump method call.<o:p></o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">2. Modify match rule with member Proper=
tiesChanged for bmc dump entry path.<o:p></o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D"><o:p>&nbsp;</o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">Tested:<o:p></o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">curl -k -H &quot;X-Auth-Token: $token&q=
uot; -X POST https://${bmc}/redfish/v1/Managers/bmc/LogServices/Dump/Action=
s/LogService.CollectDiagnosticData<o:p></o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">-d '{&quot;DiagnosticDataType&quot;: &q=
uot;Manager&quot;}'<o:p></o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D"><o:p>&nbsp;</o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">Sincerely,<o:p></o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Cali=
bri&quot;,sans-serif;color:#1F497D">Tim<o:p></o:p></span></p>
<p class=3D"MsoNormal"><b><span lang=3D"EN-US" style=3D"font-size:11.0pt;fo=
nt-family:&quot;Calibri&quot;,sans-serif">From:</span></b><span lang=3D"EN-=
US" style=3D"font-size:11.0pt;font-family:&quot;Calibri&quot;,sans-serif"> =
openbmc [mailto:openbmc-bounces+chli30=3Dnuvoton.com@lists.ozlabs.org]
<b>On Behalf Of </b>Rahul Maheshwari<br>
<b>Sent:</b> Tuesday, May 25, 2021 3:25 PM<br>
<b>To:</b> CS20 CHLi30 &lt;CHLI30@nuvoton.com&gt;<br>
<b>Cc:</b> CS20 KWLiu &lt;KWLIU@nuvoton.com&gt;; openbmc@lists.ozlabs.org<b=
r>
<b>Subject:</b> Re: [OpenBMC][Test Automation] Test Bmc Dump test failed<o:=
p></o:p></span></p>
<p class=3D"MsoNormal"><span lang=3D"EN-US"><o:p>&nbsp;</o:p></span></p>
<div>
<p class=3D"MsoNormal"><span lang=3D"EN-US">Hi Tim<o:p></o:p></span></p>
<div>
<p class=3D"MsoNormal"><span lang=3D"EN-US">`/redfish/extended/test_bmc_<sp=
an class=3D"gmail-il">dump</span>.robot` suite has usage of older xyz dump =
path. Can you make use of `redfish/managers/test_bmc_dumps.robot` suite to =
run the dump automation test? This suite&nbsp;has
 usage of recent Redfish dump uri implementation i.e. /redfish/v1/Managers/=
bmc/LogServices/Dump.&nbsp;<o:p></o:p></span></p>
</div>
<div>
<p class=3D"MsoNormal"><span lang=3D"EN-US"><o:p>&nbsp;</o:p></span></p>
</div>
<div>
<p class=3D"MsoNormal"><span lang=3D"EN-US">Thanks<o:p></o:p></span></p>
</div>
<div>
<p class=3D"MsoNormal"><span lang=3D"EN-US">Rahul<o:p></o:p></span></p>
</div>
</div>
<p class=3D"MsoNormal"><span lang=3D"EN-US"><o:p>&nbsp;</o:p></span></p>
<div>
<div>
<p class=3D"MsoNormal"><span lang=3D"EN-US">On Thu, May 20, 2021 at 10:00 A=
M CS20 CHLi30 &lt;<a href=3D"mailto:CHLI30@nuvoton.com">CHLI30@nuvoton.com<=
/a>&gt; wrote:<o:p></o:p></span></p>
</div>
<blockquote style=3D"border:none;border-left:solid #CCCCCC 1.0pt;padding:0c=
m 0cm 0cm 6.0pt;margin-left:4.8pt;margin-right:0cm">
<div>
<div>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">Hi All,<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">&nbsp;<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">We found that some of &quot;Test Bmc Dump&quo=
t; items are failed when we run test automation.<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">After checking log and source code that seems=
 relate to return type of CreateDump() in phosphor-debug-collector<o:p></o:=
p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">is not consistent with OpenBMC-Test-Automatio=
n/redfish/extended/test_bmc_dump.robot.<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">&nbsp;<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">Seems we need to modify test_bmc_dump.robot a=
bout return type from &quot;Create User Initiated Dump&quot; keyword<o:p></=
o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">from ${dump_id} to object_path according curr=
ent CreateDump() implementation in phosphor-debug-collector, am I right?<o:=
p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">&nbsp;<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">Summary:<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">CreateDump() change return type from uint32_t=
 to object_path from this commit:<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US"><a href=3D"https://apc01.safelinks.protection=
.outlook.com/?url=3Dhttps%3A%2F%2Fgithub.com%2Fopenbmc%2Fphosphor-debug-col=
lector%2Fcommit%2F6ccb50e133342d20d2758c3efc3f8b429abcbf90%23diff-6053a5177=
ee8dcedec88c55df4de554b5a54497f92b041e1585c6770236773a1&amp;data=3D04%7C01%=
7Cchli30%40nuvoton.com%7C00069a126c024c78854208d91f4e4631%7Ca3f24931d4034b4=
a94f17d83ac638e07%7C0%7C0%7C637575943652844341%7CUnknown%7CTWFpbGZsb3d8eyJW=
IjoiMC4wLjAwMDAiLCJQIjoiV2luMzIiLCJBTiI6Ik1haWwiLCJXVCI6Mn0%3D%7C1000&amp;s=
data=3DToC4ABFr7NkmzgFIJWC0HXoO6FGKe15gqZW345jvIec%3D&amp;reserved=3D0" tar=
get=3D"_blank">https://github.com/openbmc/phosphor-debug-collector/commit/6=
ccb50e133342d20d2758c3efc3f8b429abcbf90#diff-6053a5177ee8dcedec88c55df4de55=
4b5a54497f92b041e1585c6770236773a1</a><o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">&nbsp;<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">However, in OpenBMC-Test-Automation/redfish/e=
xtended/test_bmc_dump.robot<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">Verify User Initiated BMC Dump When Powered O=
ff<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">&nbsp;&nbsp;&nbsp; [Documentation]&nbsp; Crea=
te user initiated BMC dump at host off state and<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">&nbsp;&nbsp;&nbsp; ...&nbsp; verify dump entr=
y for it.<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">&nbsp;&nbsp;&nbsp; [Tags]&nbsp; Verify_User_I=
nitiated_BMC_Dump_When_Powered_Off<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">&nbsp;<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">&nbsp;&nbsp;&nbsp; Redfish Power Off&nbsp; st=
ack_mode=3Dskip<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">&nbsp;&nbsp;&nbsp; ${dump_id}=3D&nbsp; Create=
 User Initiated Dump<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">&nbsp;&nbsp;&nbsp; Check Existence Of BMC Dum=
p File&nbsp; ${dump_id}<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">&nbsp;<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">Run keyword &quot;Create User Initiated Dump&=
quot; still using dump id (uint32_t) to check create bmc dump functionality=
.<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">Thus, we got error message from &quot;Check E=
xistence Of BMC Dump File&nbsp; ${dump_id}&quot; as below:<o:p></o:p></span=
></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">Evaluating expression '/xyz/openbmc_project/d=
ump/bmc/entry/1 =3D=3D None' failed: SyntaxError: invalid syntax.<o:p></o:p=
></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">&nbsp;<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">Repo steps:<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">Run test automation: robot -t Verify_User_Ini=
tiated_BMC_Dump_When_Powered_Off redfish/extended/test_bmc_dump.robot<o:p><=
/o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">&nbsp;<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">Symptom:<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">screenshot from test automation report for yo=
u refer it.<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US"><img border=3D"0" width=3D"1040" height=3D"26=
5" id=3D"gmail-m_996545294996761257Picture_x0020_1" src=3D"cid:image001.jpg=
@01D7635D.7AA549C0"><o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">&nbsp;<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">Sincerely,<o:p></o:p></span></p>
<p class=3D"MsoNormal" style=3D"mso-margin-top-alt:auto;mso-margin-bottom-a=
lt:auto"><span lang=3D"EN-US">Tim<o:p></o:p></span></p>
</div>
<div class=3D"MsoNormal" align=3D"center" style=3D"text-align:center"><span=
 lang=3D"EN-US">
<hr size=3D"2" width=3D"100%" align=3D"center">
</span></div>
<p class=3D"MsoNormal"><span lang=3D"EN-US" style=3D"font-family:&quot;Aria=
l&quot;,sans-serif;color:gray">The privileged confidential information cont=
ained in this email is intended for use only by the addressees as indicated=
 by the original sender of this email. If you are
 not the addressee indicated in this email or are not responsible for deliv=
ery of the email to such a person, please kindly reply to the sender indica=
ting this fact and delete all copies of it from your computer and network s=
erver immediately. Your cooperation
 is highly appreciated. It is advised that any unauthorized use of confiden=
tial information of Nuvoton is strictly prohibited; and any information in =
this email irrelevant to the official business of Nuvoton shall be deemed a=
s neither given nor endorsed by
 Nuvoton. </span><span lang=3D"EN-US"><o:p></o:p></span></p>
</div>
</blockquote>
</div>
</div>
<hr align=3D"center" width=3D"100%">
<span style=3D"font-size:12pt;line-height:0.7;font-family: 'Arial'; color:#=
808080">The privileged confidential information contained in this email is =
intended for use only by the addressees as indicated by the original sender=
 of this email. If you are not the
 addressee indicated in this email or are not responsible for delivery of t=
he email to such a person, please kindly reply to the sender indicating thi=
s fact and delete all copies of it from your computer and network server im=
mediately. Your cooperation is highly
 appreciated. It is advised that any unauthorized use of confidential infor=
mation of Nuvoton is strictly prohibited; and any information in this email=
 irrelevant to the official business of Nuvoton shall be deemed as neither =
given nor endorsed by Nuvoton.
</span>
</body>
</html>

--_000_HK0PR03MB5090745CEB1A7183545A43E4CF0E9HK0PR03MB5090apcp_--

--_004_HK0PR03MB5090745CEB1A7183545A43E4CF0E9HK0PR03MB5090apcp_
Content-Type: image/jpeg; name="image001.jpg"
Content-Description: image001.jpg
Content-Disposition: inline; filename="image001.jpg"; size=45886;
	creation-date="Thu, 17 Jun 2021 02:02:26 GMT";
	modification-date="Thu, 17 Jun 2021 02:02:26 GMT"
Content-ID: <image001.jpg@01D7635D.7AA549C0>
Content-Transfer-Encoding: base64



--_004_HK0PR03MB5090745CEB1A7183545A43E4CF0E9HK0PR03MB5090apcp_--
